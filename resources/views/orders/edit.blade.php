            ...
            <div class="bg-white shadow-lg rounded-lg p-8">
                <form action="{{ route('orders.update', $order->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" id="total_price" name="total_price" step="0.01" class="w-full px-4 py-2 border-2 border-blue-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition" value="{{ old('total_price', $order->total_price) }}" placeholder="0" required>
                        @error('total_price')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    </div>
